...
void DFSClient::SetSyncTime(int sync_time) {
    this->sync_time = sync_time;
    this->client_node.SetSyncTime(sync_time);
}
...
void DFSClient::Mount(const std::string &filepath) {
    ...
    uint event_flags = IN_CREATE | IN_MODIFY | IN_OPEN;
    ...
    thread_sync = std::thread(DFSClient::SyncTimerCallback, &this->client_node);
    threads.push_back(std::move(thread_sync));
...
void DFSClient::InotifyWatcher(InotifyCallback callback,
    ...
    while (true) {
        ...
        // server cache checks in the SyncTimerCallback.
        ...
        node->InotifyWatcherCallback([&]{
            ...
            while (index < len) {
                ...
                size_t used = DFS_I_BUFFER_SIZE + event->len;
...
void DFSClient::SyncTimerCallback(DFSClientNode *node) {

    while(! node->Unmounting()) {
        node->Sync();
        std::this_thread::sleep_for(static_cast<std::chrono::milliseconds>(node->SyncTime()));
    }
...
void Usage() {
    std::cout <<
        ...
        "-r, --sync_timer <int>:  The number of milliseconds between syncs (default: 3000)\n"
        "-t, --deadline_timeout <int>:  The deadline timeout in milliseconds (default: 10000)\n"
        "-h, --help:               Show help\n"
        "\n"
        "COMMAND is one of mount|fetch|store|list|stat.\n"
        "FILENAME is the filename to fetch, store, or stat. The mount and list commands do not require a filename.\n\n";
    exit(1);
}
...
int main(int argc, char** argv) {
    ...
    std::string commands("fetch store list stat mount sync");
